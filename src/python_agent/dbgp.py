...
class StatusResponse(Response):
    ...
    def get_breaked_file_line(self):
        file = None
        line = None
        xml = self.as_xml()
        if xml.attrib['status'] == 'break' and len(xml._children) > 0:
            file = xml._children[0].attrib['filename']
            line = xml._children[0].attrib['lineno']
...
class ContextGetResponse(Response):
    ...
    def create_properties(self, property, isChild = False):
        if not isChild:
            self.properties.append(property)
        for p in property.children:
            self.create_properties(p, True)
...
class DbgpApi:
    ...
    def send_cmd(self, cmd, args = '', responseClass = Response):
        ...
        self.tryUpdateBreakedInfo(respObj)

        result.success = True
        result.data = respObj
        return result

    def getSrcCode(self, file, start_line, end_line):
        return self.send_cmd('source', ('-f %s -b %s -e %s' % (file, start_line, end_line)))

    def getSrcCodeBlock(self, prevLine = 10, afterLine = 20):
        result = self.getSrcCode(self.breakedFile, self.breakedLine - prevLine, self.breakedLine + afterLine)
        if result.success:
            codeBase64 = result.data.as_xml().text
            result.data = (codeBase64, self.breakedFile, self.breakedLine)

        return result

    def tryUpdateBreakedInfo(self, response):
        if isinstance(response, StatusResponse):
            self.lastStatus = response.as_xml().attrib['status']

            file, line = response.get_breaked_file_line()
            if file:
                self.breakedFile = file
                self.breakedLine = line
    ...
    def eval(self, code):
        """Tell the debugger to start or resume execution."""
    ...
    def stop(self):
        ...
        result = self.send_cmd('stop', '', StatusResponse)
        if result.success:
            self.conn.close()
            result.message = 'debug stopped!'
        return result
    ...
    def context_get(self, context = 0, stackDepth = 0):
        """Get the context variables.
        """
        return self.send_cmd('context_get', '-d %s -c %s' % (stackDepth, context), ContextGetResponse)
